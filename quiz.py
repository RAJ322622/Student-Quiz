...
def register_user(username, password, role):
    if username == PROFESSOR_USERNAME or role == "professor":
        st.error("Professor registration is not allowed.")
        return
...
menu = ["Login", "Register (Students Only)", "Take Quiz", "Change Password", "Download Results (Prof Only)", "Video Record (Prof Only)"]
choice = st.sidebar.selectbox("Menu", menu)

if choice == "Register (Students Only)":
    ...
    if not st.session_state.logged_in:
        ...
        if username == PROFESSOR_USERNAME:
            st.warning("Professors are not allowed to take the quiz.")
        else:
            score = 0
    ...
    if not st.session_state.logged_in:
        ...
        if role != "professor":
            ...
            if os.path.exists(PROF_CSV_FILE):
                ...
                st.success("Status: File ready for download.")
            else:
                st.warning("No result file found.")

elif choice == "Video Record (Prof Only)":
    if not st.session_state.logged_in:
        st.warning("Please login first!")
    else:
        username = st.session_state.username
        ...
        if role != "professor":
            ...
            st.info("This is for monitoring purposes only. It records a live video feed from the webcam.")
            webrtc_streamer(
