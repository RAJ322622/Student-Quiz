...
if choice == "Register":
    ...
    if st.button("Register"):
        if not username or not password or not email:
            ...
            # Send registration email (dummy placeholder)
            try:
                import smtplib
                from email.mime.text import MIMEText

                msg = MIMEText(f"Hi {username},\n\nYour registration to the Secure Quiz App is successful!")
                msg["Subject"] = "Registration Successful - Secure Quiz App"
                msg["From"] = "yourapp@example.com"
                msg["To"] = email

                # Replace with actual SMTP details
                with smtplib.SMTP("smtp.gmail.com", 587) as server:
                    server.starttls()
                    server.login("yourapp@example.com", "yourpassword")  # Replace with actual credentials
                    server.send_message(msg)

                st.success("Registration email sent.")
            except Exception as e:
                st.warning("Registered, but email could not be sent. Check SMTP setup.")
